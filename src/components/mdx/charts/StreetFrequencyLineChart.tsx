...
import { TrendUp, CornersOut, CornersIn } from '@phosphor-icons/react';
...
export default function StreetFrequencyLineChart() {
  ...
  const { series, years, minYear, maxYear, maxY } = useMemo(() => {
    ...
    const validSeries = (rawData as any[]).filter(s => s.type === 'scatter' && s.x && s.y);
  ...
  const generatePath = (points: Point[]) => {
    ...
    if (points.length === 1 && points[0]) return `M ${mapX(points[0].x)},${mapY(points[0].y)}`;
    ...
    >
      ...
      <div className="mb-8 pr-10">
        <h3 className="text-xl sm:text-2xl font-semibold text-slate-800 dark:text-slate-100 flex items-center justify-start gap-2">
          <TrendUp weight="duotone" className="text-accent-500" />
      ...
      <div className="relative w-full aspect-[2/1] mt-6 select-none bg-slate-50/50 dark:bg-slate-800/10 rounded-xl overflow-hidden">
        ...
        >
            ...
            return (
              <text 
                ...
                className={clsx(
                  ...
                  isHovered ? "fill-accent-500 font-bold" : "fill-slate-500 dark:fill-slate-300"
                )}
                textAnchor="middle"
              >
                {year}
          ...
          {series.map((s, sIdx: number) => {
            const points = s.x.map((x: number, i: number) => ({ x, y: s.y[i] }));
            const d = generatePath(points);
            const isHovered = hoveredStreet === s.name;
            const dim = hoveredStreet !== null && !isHovered;

            return (
              <g key={s.name} className="transition-opacity duration-300" style={{ opacity: dim ? 0.2 : 1, zIndex: isHovered ? 10 : 1 }}>
          ...
          {activeYearIndex !== null && (
            ...
            >
              ...
              <div className="flex flex-col gap-1 max-h-[150px] overflow-y-auto no-scrollbar">
                ...
                {[...series].sort((a,b) => b.y[activeYearIndex] - a.y[activeYearIndex]).slice(0, 7).map(s => (
                  <div key={s.name} className="flex justify-between items-center text-xs gap-4">
                    <div className="flex items-center gap-1.5 flex-1 truncate">
                      <div className="w-2 h-2 rounded-full flex-shrink-0" style={{ backgroundColor: s.color }} />
                      <span className={clsx(
                        "truncate text-slate-600 dark:text-slate-300",
                        hoveredStreet === s.name && "font-bold !text-slate-900 dark:!text-white"
                      )}>
                        {s.name}
                      </span>
                    </div>
                    <span className={clsx(
                      ...
                      hoveredStreet === s.name && "font-bold text-accent-500"
                    )}>
                      {s.y[activeYearIndex]}
                    </span>
                  </div>
      ...
      <div className="mt-6 flex flex-wrap gap-2 sm:hidden">
        {series.map(s => (
           <span
             ...
             onClick={() => setHoveredStreet(hoveredStreet === s.name ? null : s.name)}
           >
